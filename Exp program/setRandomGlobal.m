function outPat = setRandomGlobal(inputPat, dNum1, dNum2, targetQuad)
...
temp(quadInd(setdiff(1:4,targetQuad),:)) = 0; % set quadrant to zero
...
while sum(temp>0) ~= 17
    temp(quadInd(setdiff(1:4,targetQuad),:)) = 0; % set quadrant to zero
    temp(pat_tpos) = 1001; % add target back in
    for q = setdiff(1:4,targetQuad) % adds distractors to all quadrants except the target 
        for d = 1:4
